...
int Thread::getRemainTimeSlice()
{
    return remainTimeSlice;
}

void Thread::setRemainTimeSlice(int rts)
{
    remainTimeSlice = rts;
}

void Thread::subTimeSlice(int ts){
    remainTimeSlice-=ts;
...
void Thread::updatePriority(int Num) //update the thread's priority
{
    if(Num>0){
        priority+=Num;
         cout << name << "'s priority is" << priority << endl;
    }
    else {
        priority+=Num;
         cout << name << "'s priority is" << priority << endl;
...
Thread::Begin ()
{
    ...
    DEBUG(dbgThread, "Beginning thread: " << name);
     ...
     cout << this->getName() << " is begun" << endl;
...
Thread::Finish ()
{
    ...
    DEBUG(dbgThread, "Finishing thread: " << name);
     cout << this->getName() << " is finished" << endl;
...
//cout << this->getName() << " is yielded and next thread is " << nextThread->getName() << endl;
    if (nextThread != this) 
    {
        if(this->getPriority() >= nextThread->getPriority() && this->remainTimeSlice !=0){
            //thread give up
    ...
    this->setRemainTimeSlice(INITSLICE);
...
Thread::Sleep (bool finishing)
{
	...
	kernel->interrupt->Idle();	
     ...
     cout << this->getName() << " is slept" << endl;
...
/**
    ...
    t3->setPriority(23);
    t3->Fork((VoidFunctionPtr) SimpleThread, (void *) 4);
    
    Thread *t4 = new Thread("Thread5");
    t4->setPriority(35);
    t4->Fork((VoidFunctionPtr) SimpleThread, (void *) 5);
  **/
